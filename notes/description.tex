...
(HIT), which are inductive types that also allow for higher path
constructors, by treating them as freely generated cubical sets. When mapping
into a higher inductive type, one has to respect the coherences introduced by
these path constructors, which means that one has to show that certain cells in
...
points. One proof goal one could face when mapping into \cset{Int} is that one
...
into the set of poset maps that its representing with
