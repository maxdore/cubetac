...
-- -- allFormulas n m = map (map Clause) (replicateM n (ps [1..n]))
...
disj2subst :: Disj -> Subst
disj2subst (Disj (m , rs)) = Map.fromList (map (\v -> (v , Vert (map (evalFormula v) rs))) (create1ConnPoset m))
  where
  evalFormula :: Vert -> Formula -> Endpoint
  evalFormula (Vert x) (Endp e) = e
  evalFormula (Vert x) (Clause is) = case elemIndex I1 x of
    Just i -> fromBool (i+1 `elem` is)
    Nothing -> I0

subst2disj :: Subst -> Disj
subst2disj sigma = Disj (domdim sigma , map reconstrForm [1..coddim sigma])
  where
  reconstrForm :: IVar -> Formula
  reconstrForm i | toBools (fromJust (Map.lookup (Vert (replicate (domdim sigma) I0)) sigma))!!(i-1) == I1 = Endp I1
                 | otherwise = Clause [ j | j <- [1..domdim sigma] , toBools (fromJust (Map.lookup (baseVert (domdim sigma) j) sigma))!!(i-1) == I1 ]
...
instance Bs Disj where
  ...
  isFace (Disj (m , rs)) = case elemIndex (Endp I0) rs of
    Just i -> Just (i+1,I0)
  ...
  -- map subst2disj (getSubsts (Map.fromList $ map (\v -> (v , createPoset n)) (create1ConnPoset m)))

instance Rs Disj PSubst where
  allPConts _ m n = [ create1ConnPSubst m n ]
  unfold = (map subst2disj) . getSubsts
  combine = combineSubsts . (map disj2subst)
...
conj2subst :: Conj -> Subst
conj2subst (Conj (m , rs)) = Map.fromList (map (\v -> (v , Vert (map (evalFormula v) rs))) (create1ConnPoset m))
  where
  evalFormula :: Vert -> Formula -> Endpoint
  evalFormula (Vert x) (Endp e) = e
  evalFormula (Vert x) (Clause is) = case elemIndex I1 x of
...
subst2conj :: Subst -> Conj
subst2conj sigma = Conj (domdim sigma , map reconstrForm [1..coddim sigma])
  where
  reconstrForm :: IVar -> Formula
  reconstrForm i | toBools (fromJust (Map.lookup (Vert (replicate (domdim sigma) I0)) sigma))!!(i-1) == I1 = Clause []
                 | and (Map.map (\y -> (toBools y)!!(i-1) == I0) sigma) = Endp I0
                 | otherwise = Clause [ j | j <- [1..domdim sigma] , toBools (fromJust (Map.lookup (baseVert (domdim sigma) j) sigma))!!(i-1) == I1 ]
...
instance Bs Conj where
  ...
  isFace (Conj (m , rs)) = case elemIndex (Endp I0) rs of
    Just i -> Just (i+1,I0)
    Nothing -> case elemIndex (Endp I1) rs of
      Just i -> Just (i+1,I1)
      Nothing -> Nothing
  ...
  allConts m n = map (\rs -> Conj (m , rs)) (map (map Clause) (replicateM n (ps [1..m])))
    ...
    -- map subst2conj (getSubsts (Map.fromList $ map (\v -> (v , createPoset n)) (create1ConnPoset m)))

instance Rs Conj PSubst where
  allPConts _ m n = [ create1ConnPSubst m n ]
  unfold = (map subst2conj) . getSubsts
  combine = combineSubsts . (map conj2subst)
